...
class MoEAlltoAllTokenDispatcher(MoETokenDispatcher):
    ...
    def __init__(
        ...
        comm_event_lst: torch.cuda.Event,
        ...
        self.comm_event_lst = comm_event_lst
        self.local_expert_indices = local_expert_indices
        assert (
    ...
    def preprocess(self, indices: torch.Tensor) -> torch.Tensor:
        ...
        if self.drop_and_pad:
            ...
            self.capacity = self.probs.size(1) // 2
    ...
    def token_permutation(
        ...
        if self.cuda_sync_point == "before_ep_alltoall":
            torch.cuda.current_stream().synchronize()
        chunked_permutated_local_input_tokens_lst = torch.chunk(permutated_local_input_tokens, 2, 0)
        global_input_tokens_lst = []
        self.comm_stream.wait_stream(torch.cuda.current_stream())
        with torch.cuda.stream(self.comm_stream):
            for i in range(2):
                global_input_tokens = tensor_parallel.all_to_all(
                    parallel_state.get_expert_model_parallel_group(),
                    chunked_permutated_local_input_tokens_lst[i],
                    self.output_splits,
                    self.input_splits,
                )
                ...
                # print("Global:", global_input_tokens.shape)
                # print("How to check:", self.num_global_tokens_per_local_expert_cpu)
                ...
                global_input_tokens_lst.append(global_input_tokens)
            ...
            return global_input_tokens_lst, tokens_per_expert

    def token_unpermutation(
        self, hidden_states_lst: torch.Tensor, bias: torch.Tensor = None
        ...
        permutated_local_input_tokens_lst = []
        with torch.cuda.stream(self.comm_stream):
            for i, hidden_states in enumerate(hidden_states_lst):
                self.comm_stream.wait_event(self.comm_event_lst[0])
                if self.num_local_experts > 1:
                ...
                permutated_local_input_tokens_lst.append(permutated_local_input_tokens)
        torch.cuda.current_stream().wait_stream(self.comm_stream)
        permutated_local_input_tokens = torch.cat(permutated_local_input_tokens_lst, dim=0)
        # print(permutated_local_input_tokens.shape)
